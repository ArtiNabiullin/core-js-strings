...
function findLongestWord(sentence) {
  ...
  const words = sentence.replace(/[^a-zA-Zа-яА-ЯёЁ0-9\s]/g, '').split(/\s+/);
...
function getCardId(value) {
  ...
  const ranksIndex = ranks.indexOf(rank);
  const suitsIndex = suits.indexOf(suit);

  return suitsIndex * 13 + ranksIndex;
}
